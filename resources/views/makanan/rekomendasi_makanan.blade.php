         ...
         Mencari rekomendasi terbaik untukmu...
                ...
                fetch("{{ route('makanan.get_rekomendasi') }}", {
                    ...
                    .then(data => {
                        ...
                        // 👇 Ambil items dengan fallback ke array langsung
                        ...
                        if (Array.isArray(items) && items.length > 0) {
                            items.forEach(item => {
                                ...
                                // ✅ Format harga aman untuk "20000" atau "20.000"
                                const hargaStr = String(item.harga ?? '0');
                                const hargaNum = Number(hargaStr.replace(/\./g, '').replace(',', '.')) || 0;
                                const hargaFmt = hargaNum.toLocaleString('id-ID');
...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden group flex flex-col">
  ...
  <div class="p-3 md:p-5 flex flex-col flex-grow">
    ...
    <div class="mt-2 md:mt-3 flex items-center justify-between border-t pt-2 md:pt-3">
      <div class="text-lime-700 font-semibold md:font-bold text-base md:text-lg">Rp. ${hargaFmt}</div>
...
<div class="col-span-2 md:col-span-3 lg:col-span-4 text-center text-gray-500 text-lg py-10">
  Tidak ada rekomendasi yang sesuai. Silakan ubah kriteria Anda.
                    ...
                    .catch(err => {
                        console.error(err);
                        hasilRekomendasiDiv.innerHTML =
                            `<div class="col-span-full text-red-600 text-center text-lg py-10">
